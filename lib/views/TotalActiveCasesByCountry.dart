...
class TotalActiveCasesByCountry extends StatefulWidget {
  @override
  _TotalActiveCasesByCountryState createState() => _TotalActiveCasesByCountryState();
}

class _TotalActiveCasesByCountryState extends State<TotalActiveCasesByCountry> {
  ...
  void _getTotalCasesDataByCountry()async{
    final cases = await apiService.getEndPointDataForQuery(accessToken: _accessToken, endpoint: Endpoint.active, queryCountry: _currentSelectedCountry);
    print(cases);
    setState(() {
      _cases = cases;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Cases Still ACTIVE in a country", style: TextStyle(fontSize: 16),),
