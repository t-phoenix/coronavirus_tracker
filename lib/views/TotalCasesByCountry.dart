...
class TotalCasesByCountry extends StatefulWidget {



  @override
  _TotalCasesByCountryState createState() => _TotalCasesByCountryState();
}



class _TotalCasesByCountryState extends State<TotalCasesByCountry> {
  ...
  void _getTotalCasesDataByCountry()async{
    final cases = await apiService.getEndPointDataForQuery(accessToken: _accessToken, endpoint: Endpoint.cases, queryCountry: _currentSelectedCountry);
    print(cases);
    setState(() {
      _cases = cases;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("TOTAL COVID Cases by Country", style: TextStyle(fontSize: 16),),
