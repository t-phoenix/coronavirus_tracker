...
class TotalDeathsByCountry extends StatefulWidget {
  @override
  _TotalDeathsByCountryState createState() => _TotalDeathsByCountryState();
}

class _TotalDeathsByCountryState extends State<TotalDeathsByCountry> {
  ...
  void _getTotalCasesDataByCountry()async{
    final cases = await apiService.getEndPointDataForQuery(accessToken: _accessToken, endpoint: Endpoint.deaths, queryCountry: _currentSelectedCountry);
    print(cases);
    setState(() {
      _cases = cases;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("TOTAL DEATH in a country", style: TextStyle(fontSize: 16),),
